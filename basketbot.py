...
async def playercareerstats(ctx, *args):
    ...
    if not players == None:  # If the list of players is not empty
        ...
        if len(playerIds) > 1:  # If there are multiple players with the same name then list out all the players
            embed = discord.Embed(title="Multiple Players",
                                  description="There are multiple players with the name \"" + name.strip() + "\":",
                                  color=0x595959)
...
async def player(ctx, *args):
    ...
    embed = discord.Embed(title="Player Not Found",
                          description="Double check that the player name is spelled correctly, or try using %playercareerstats [player].",
                          color=0x595959)
    ...
    if players is not None:  # If the list of players is not empty
        ...
        if len(playerIds) > 1:  # If there are multiple players with the same name then list out all the players
            ...
            height = stats["HEIGHT"].split("-")
            embed = discord.Embed(title=playerNames[0],
                                  description=stats["SEASON_ID"] + "\n" + stats["TEAM_CITY"] + " " + stats[
                                      "TEAM_NAME"] + " | #" + stats["JERSEY"] + " | "
                                              + stats["POSITION"] + "\n" + height[0] + "'" + height[1] + "\"" + " | " +
                                              stats["WEIGHT"] + " lbs", color=stats["TEAM_COLOR"])
            embed.set_thumbnail(url=nba.getPlayerHeadshotURL(playerIds[0]))
            ...
            embed.add_field(name="Points", value="**" + str(stats["PTS"]) + "** (" + str(stats["PPG"]) + " PPG)",
                            inline=False)
            embed.add_field(name="Rebounds", value="**" + str(stats["REB"]) + "** (" + str(stats["RPG"]) + " RPG)",
                            inline=False)
            embed.add_field(name="Assists", value="**" + str(stats["AST"]) + "** (" + str(stats["APG"]) + " APG)",
                            inline=False)
            embed.add_field(name="Blocks", value="**" + str(stats["BLK"]) + "** (" + str(stats["BPG"]) + " BPG)",
                            inline=False)
            embed.add_field(name="Steals", value="**" + str(stats["STL"]) + "** (" + str(stats["SPG"]) + " SPG)",
                            inline=False)
    await ctx.send(embed=embed)
...
async def team(ctx, *args):
    ...
    embed = discord.Embed(title="Team Not Found", description="Double check that the team name is spelled correctly.",
                          color=0x595959)
    ...
    if teams is not None:
        ...
        if len(teamIds) > 1:  # If there are multiple teams with the same name
            embed = discord.Embed(title="Multiple Teams",
                                  description="There are multiple teams with the name \"" + name.strip() + "\":",
                                  color=0x595959)
            ...
            stats = await nba.getTeamSeasonStatsByID(teamIds[0])
            embed = discord.Embed(title=teamNames[0],
                                  description=stats["SEASON_ID"] + "\n" + make_ordinal(stats["CONF_RANK"]) + " in " +
                                              stats["TEAM_CONFERENCE"]
                                              + " | " + make_ordinal(stats["DIV_RANK"]) + " in " + stats[
                                                  "TEAM_DIVISION"] + "\n", color=stats["TEAM_COLOR"])
            embed.set_thumbnail(url=nba.getTeamLogoURL(teamIds[0]))
            embed.add_field(name="Points Per Game", value="**" + str(stats["PPG"]) + "** (" + make_ordinal(
                stats["PTS_RANK"]) + " in the league)", inline=False)
            embed.add_field(name="Rebounds Per Game", value="**" + str(stats["RPG"]) + "** (" + make_ordinal(
                stats["REB_RANK"]) + " in the league)", inline=False)
            embed.add_field(name="Assists Per Game", value="**" + str(stats["APG"]) + "** (" + make_ordinal(
                stats["AST_RANK"]) + " in the league)", inline=False)
            embed.add_field(name="Opponent Points Per Game", value="**" + str(stats["OPPG"]) + "** (" + make_ordinal(
                stats["OPPG_RANK"]) + " in the league)", inline=False)
    await ctx.send(embed=embed)
